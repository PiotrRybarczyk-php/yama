...
function qanim() {
    ...
    for (qanim_a; qanim_a < element.length; qanim_a++) {
        animposition[qanim_a] = (element[qanim_a].getBoundingClientRect().top + window.scrollY) - 600;
        animoffset[qanim_a] = animposition[qanim_a] + (element[qanim_a].offsetHeight + 300);
        ...
        for (qanim_b; qanim_b < element.length; qanim_b++) {
            if (animcheck[qanim_b] == true && animdone[qanim_b] == false) {
                if (animcounter[qanim_b] >= 100) {
                    ...
                    } else if (element[qanim_b].classList.contains('qa_fade-left') == true) {
                        element[qanim_b].style.left = 0 + "%";
                    } else if (element[qanim_b].classList.contains('qa_fade-down') == true) {
                        element[qanim_b].style.top = 0 + "%";
                    } else if (element[qanim_b].classList.contains('qa_fade-up') == true) {
                        element[qanim_b].style.bottom = 0 + "%";
                    }
                    animdone[qanim_b] = true;
                } else {
                    animcounter[qanim_b] = animcounter[qanim_b] + ((Math.cos(animcounter[qanim_b] * rad) * mul_speed) + min_speed); // oh yeah... that makes animation smooth baby ;p
...
window.onscroll = function () {
    for (qanim_a; qanim_a < element.length; qanim_a++) {
        if (window.scrollY >= animposition[qanim_a] && window.scrollY <= animoffset[qanim_a] && animcheck[qanim_a] == false) { animcheck[qanim_a] = true; } // checks if you scrolled to element position
    }
    qanim_a = 0;
};
